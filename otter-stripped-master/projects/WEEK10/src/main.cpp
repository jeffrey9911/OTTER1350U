...
std::string windowTitle = "INFR1350U-Jeffrey-Li-100712344";
...
void CreateScene() {
	...
	if (loadScene) {
		...
		Shader::Sptr waveShader = ResourceManager::CreateAsset<Shader>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shaders/vertex_shader_tut.glsl" },
			{ ShaderPartType::Fragment, "shaders/fragment_shaders/frag_shader_tut.glsl" }
		});

		// Load in the meshes
		MeshResource::Sptr monkeyMesh = ResourceManager::CreateAsset<MeshResource>("Monkey.obj");
		//MeshResource::Sptr waveMesh = ResourceManager::CreateAsset<MeshResource>("sphere.obj");
		MeshResource::Sptr waveMesh = ResourceManager::CreateAsset<MeshResource>("complexPlane.obj");
		...
		Texture2D::Sptr		waveTex = ResourceManager::CreateAsset<Texture2D>("textures/waveTex.png");
		Texture2D::Sptr		heightTex = ResourceManager::CreateAsset<Texture2D>("textures/heightmap.bmp");
		...
		Material::Sptr waveMaterial = ResourceManager::CreateAsset<Material>(waveShader);
		{
			waveMaterial->Name = "Wave Material";
			waveMaterial->Set("u_Material.Diffuse", waveTex);
			waveMaterial->Set("myTextureSampler", heightTex);
			waveMaterial->Set("u_Material.Shininess", 0.1f);
			waveMaterial->Set("u_Material.Steps", 4);
		}  
		...
		GameObject::Sptr wavePlane = scene->CreateGameObject("Wave Plane");
		{
			wavePlane->SetPostion(glm::vec3(0.0f, 0.0f, 2.0f));
			wavePlane->SetScale(glm::vec3(3.0f, 3.0f, 1.0f));

			// Create and attach a RenderComponent to the object to draw our mesh
			RenderComponent::Sptr renderer = wavePlane->Add<RenderComponent>();
			renderer->SetMesh(waveMesh);
			renderer->SetMaterial(waveMaterial); 
		}
