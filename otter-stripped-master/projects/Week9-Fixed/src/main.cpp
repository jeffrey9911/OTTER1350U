...
std::string windowTitle = "INFR-1350U-100712344-Jeffrey-Li";
...
int main() {
	...
	if (loadScene) {
		...
		// Reflective shader by spec of white and black
		Shader::Sptr refBySpecShader = ResourceManager::CreateAsset<Shader>(std::unordered_map<ShaderPartType, std::string>{
			{ ShaderPartType::Vertex, "shaders/vertex_shader.glsl" },
			{ ShaderPartType::Fragment, "shaders/frag_environment_reflective_myShader.glsl" }
		});
		...
		Texture2D::Sptr    boxSpecular = ResourceManager::CreateAsset<Texture2D>("textures/box-specular.png");
		...
		// Reflective material by specular for box
		Material::Sptr refbySpecMaterial = ResourceManager::CreateAsset<Material>();
		{
			refbySpecMaterial->Name = "ReflectiveBox";
			refbySpecMaterial->MatShader = refBySpecShader;
			refbySpecMaterial->Texture = boxTexture;
			refbySpecMaterial->SpecularTex = boxSpecular;
		}
		...
		GameObject::Sptr square = scene->CreateGameObject("Square");
		{
			...
			square->SetPostion(glm::vec3(4.5f, 4.5f, 4.5f));
			square->SetRotation(glm::vec3(0.0f, 45.0f, 0.0f));
			// Scale down the plane
			square->SetScale(glm::vec3(0.8));

			// Create and attach a render component
			RenderComponent::Sptr renderer = square->Add<RenderComponent>();
			renderer->SetMesh(planeMesh);
			renderer->SetMaterial(refbySpecMaterial);
	...
	while (!glfwWindowShouldClose(window)) {
		...
		GameObject::Sptr theSquare = scene->FindObjectByName("Square");
