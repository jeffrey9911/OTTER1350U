...
int main() {
	...
	if (loadScene) {
		...
		Guid boxTexture = ResourceManager::CreateTexture("textures/box.bmp");
		Guid monkeyTex  = ResourceManager::CreateTexture("textures/monkey-uvMap.png");

		// my stuff
		Guid boyMesh = ResourceManager::CreateMesh("myOBJ.obj");
		Guid boyTex = ResourceManager::CreateTexture("textures/myTex.jpg");
		...
		// my boy material
		MaterialInfo::Sptr boyMaterial = std::make_shared<MaterialInfo>();
		boyMaterial->Shader = scene->BaseShader;
		boyMaterial->Texture = ResourceManager::GetTexture(boyTex);
		boyMaterial->Shininess = 1.0f;
		scene->Materials[boyMaterial->GetGUID()] = boyMaterial;
		// my boy material end
		...
		// render my obj
		RenderObject myBoy = RenderObject();
		myBoy.Position = glm::vec3(0.0f, 3.0f, 0.0f);
		myBoy.Mesh = ResourceManager::GetMesh(boyMesh);
		myBoy.Material = boyMaterial;
		myBoy.Rotation.x = 90.0f;
		myBoy.Scale = glm::vec3(0.2f, 0.2f, 0.2f);
		myBoy.Name = "boy 1";
		scene->Objects.push_back(myBoy);
	...
	// find my
	RenderObject* boy1 = scene->FindObjectByName("boy 1");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (isDebugWindowOpen) {
			...
			if (DrawSaveLoadImGui(scene, scenePath)) {
				...
				//my boy
				boy1 = scene->FindObjectByName("boy 1");
			}
			ImGui::Separator();
		}
		...
		if (isRotating) {
			...
			boy1->Rotation += glm::vec3(0.0f, 0.0f, dt * 180.0f);
		}
